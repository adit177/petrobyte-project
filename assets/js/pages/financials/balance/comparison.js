...
const tableFormation = function (element, data) {
  ...
  redData = $R_table_obj.$_simple(data, pb.fin.balance.comparison.t.transact);
...
const loadsTarget = function (element, event) {
  ...
  const _load_table = (element, event) => {
    ...
    const _process_a = (data) => {
      ...
      //PB_extend_foreign.$_remote(exT.foreign.whole, data, [['transact', 'dates', '1', '0'], ['transact', "ledgers", '2', '0'], ['transact', 'Txn_type', '3', '0']]);
      // rendering data into table using above profile.
...
const actionsTarget = function (element, event) {
  console.log("in actions", element);
...
const pageOpen = (data) => {
  ...
  const _process_update_business_info = (data) => {
    ...
    _sheetLE.sheet.querySelectorAll(".fy_year1").forEach((child) => {
      child.innerText = 'FY ' + StateData.years['fy1'];
    })

    _sheetLE.sheet.querySelectorAll(".fy_year2").forEach((child) => {
      child.innerText = 'FY ' + StateData.years['fy2'];
    })
