...
const tableFormation = function (element, data) {
  ...
  redData = $R_table_obj.$_simple(data, pb.fin.balance.tformat.t.transact);
...
const loadsTarget = function (element, event) {
  ...
  const _load_table = (element, event) => {
    ...
    const _process_a = (data) => {
      ...
      // extend.foreign.$_remote(exT.foreign.whole, foreignData, [['transact', 'dates', '1', '0'], ['transact', "ledgers", '2', '0'], ['transact', 'Txn_type', '3', '0']]);
      // rendering data into table using above profile.
