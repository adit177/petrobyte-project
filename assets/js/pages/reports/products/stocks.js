...
const tab_location = function (element, data, event) {
  ...
  const _process_tab = (data) => {
    switch (event.value) {
      ...
      case pb.rpt.products.stocks.p.location:
        return tab_location(element, data.data, event);
...
const reportsTarget = function (element, event, hybrid = false) {
  ...
  const report_item = function (element, event) {
    const _process = (data) => {
      ...
      _return &&= _process_report_stock_item(data.data);
    ...
    const _process_report_stock_item = (data) => {

      console.log(data.table);
      extend.foreign.$_table({...data.table, ...getStateData()},
        ["parent", "items", "item_id", 1],
        ["child", "stores", "store_id", 1]);
      ...
      redData = $R_table_obj.$_nested(data.table, pb.rpt.products.stocks.p.item,
        ["Store", "Stocks", "Units"]);
  ...
  const report_location = function (element, event) {
    const _process = (data) => {
      let _return = true;

      _return &&= _process_header_posting(data.data);
      _return &&= _process_report_stock_location(data.data);
    ...
    const _process_report_stock_location = (data) => {
  ...
  //  if (common_exe() == false) return false;
  if (hybrid) {
  ...
  switch (event.value) {
    ...
    case pb.rpt.products.stocks.p.location:
      return report_location(element, event);
...
const backsTarget = function (element, event) {
  ...
  const _tableEl = _passedEl.querySelector('table');
...
// Public methods
export const RPST_pageOpen = (data, type) => {
