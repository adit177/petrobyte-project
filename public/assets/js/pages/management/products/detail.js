...
const navtabsTarget = function (element, event, autoCall = false) {
  ...
  const navtab_overview = function (element, event) {
    ...
    const _process_sales_balance = (data) => {
   ...
   * @returns {boolean}
   */
  const navtab_sales = function (element, event) {
    ...
    const _process = (data) => {
      ...
      let _return = true;
      _return &&= _process_cards_render(data);
    ...
    const _process_cards_render = (data) => {
      let templateEle = _panelLE.template.querySelector(querySA(atr.core.template, "breif-overview-sales", 'div'));
      let targetEle = element.querySelector(querySA(atr.core.append, "breif-overview-sales"));
      targetEle.innerHTML = '';
      for (const key in data['insight_info']) {
        let emp1 = templateEle.cloneNode(true)
        emp1.innerHTML = extend.append.$_single_on_innerHTML(emp1, data['insight_info'][key])
        emp1.classList.remove('d-none')
        targetEle.append(emp1)
      }
      return true;
    }
    const _process_chart_render = (data) => {
      ...
      console.log(data);
      //chart 1
      redData = Option_charts.createDataset(data['charts']);
    ...
    const _process_table_render = (data) => {
      ...
      // rendering data into table using above profile.
      PB_defined.$_append_table_simple(tableEle, data.table, pb.mng.products.detail.t.sales);
  ...
  const navtab_purchases = function (element, event) {
    console.log(element, event);
    console.log(_panelLE.nav)
    //check the state if data is already loaded then no need to load the data again
    if (element.getAttribute(atr.load.tab) === '1') {
    ...
    const _process_cards_render = (data) => {
      let templateEle = _panelLE.template.querySelector(querySA(atr.core.template, "breif-overview-purds", 'div'));
      let targetEle = element.querySelector(querySA(atr.core.append, "breif-overview-purds"));
      targetEle.innerHTML = '';
      for (const key in data['insight_info']) {
        let emp1 = templateEle.cloneNode(true)
        emp1.innerHTML = extend.append.$_single_on_innerHTML(emp1, data['insight_info'][key])
    ...
    const _process_table_render = (data) => {
      ...
      // rendering data into table using above profile.
      PB_defined.$_append_table_simple(tableEle, data.table, pb.mng.products.detail.t.purchases);
  ...
  //  }
  const navtab_config = function (element, event) {
    const _process = (data) => {
      data = data.data

      let _return = true;
      _return &&= _process_form_render(data);

      return _return;
    }
    const _process_form_render = (data) => {
      const formEle = element.querySelector('form');
      const data1 = getStateData(["categories", "units"])
      console.log(data1, data, formEle);
      // render the response data (if required)
      $R_form_obj.$global({...data, ...data1}, formEle, rdR.form.method.repeater);
  ...
  switch (event.value) {
    ...
    case pb.mng.products.detail.p.overview:
      return navtab_overview(element, event);

    case pb.mng.products.detail.p.sales:
      return navtab_sales(element, event);

    case pb.mng.products.detail.p.purchases:
      return navtab_purchases(element, event);

    case pb.mng.products.detail.p.config:
      return navtab_config(element, event);
...
export const MPD_Requests = (data, type) => {
