...
const stacksTarget = (element, event) => {
  ...
  switch (event.value) {
    case pb.sam.extends.search.p:
      return _abcd();
  }
}

const pageBack = (data) => {
  toastr.info('bank clicked', 'pageBack - search');
  return Promise.resolve(true);
}

const pageClear = (data) => {
  toastr.info('clear state data.', 'pageClear - search');
  return Promise.resolve(true);
}
...
export const SES_Requests = (data, type) => {
  console.log(data, type);
  toastr.success('SES_Requests', 'Request Type: ' + type);
  switch (type) {
