...
const reportsTarget = function (element, event, hybrid = false) {
  ...
  const report_view = function (element, event) {
    const _process = (data) => {
      let _return = true;

      _return &&= _process_key_value_match(data.data);
      _return &&= _process_header_posting(data.data);
      _return &&= _process_report_bill_view(data.data);
    ...
    const _process_report_bill_view = (data) => {
  ...
  const report_statement = function (element, event) {
    const _process = (data) => {
      ...
      _return &&= _process_key_value_match(data.data);
      _return &&= _process_header_posting(data.data);
      _return &&= _process_report_bill_statement(data.data);
    ...
    const _process_key_value_match = (data) => {
      extend.foreign.$_table({...data, ...getStateData()}, [
        ["table", "dates", "date_id", 0],
        ["table", "dates", "period_start_id", 0],
        ["table", "dates", "period_end_id", 0],
    ...
    const _process_report_bill_statement = (data) => {
  ...
  const report_customer = function (element, event) {
    const _process = (data) => {
      ...
      _return &&= _process_key_value_match(data.data);
      _return &&= _process_header_posting(data.data);
      _return &&= _process_report_bill_customer(data.data);

      return _return;
    }
    const _process_key_value_match = (data) => {
      extend.foreign.$_table({...data, ...getStateData()}, [
        ["table", "dates", "bill_date_id", 0],
        ["table", "dates", "period_start_id", 0],
        ["table", "dates", "period_end_id", 0],
      ])
      console.log(data);
    ...
    const _process_report_bill_customer = (data) => {
  ...
  switch (event.value) {
    case pb.rpt.customers.bills.p.view:
      return report_view(element, event);
...
export const RCB_pageOpen = (data, type) => {
