...
const navtabsTarget = (element, event, onPageLoad = false) => {
  const summary_navtab = (_element, _event, _onPageLoad) => {
    ...
    const _process_a = (data) => {
      // finding template
      const template = _directLE.templates.querySelector(querySA(atr.core.template, event.value));
...
export const DSP_navtabs = function (_event) {
  return eleCheck() ? navtabsTarget(get_callEl(), _event) : false;
};
// fetching all upcoming required details
export const DSP_Requests = function (data) {
